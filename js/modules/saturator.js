...
// ----------------------------------------------------------------
// UI personalizada del Saturador (curva de transferencia + knobs)
// ----------------------------------------------------------------
export function buildUI(mod) {
  ...
  canvas.className = 'saturator-curve-canvas';
  canvas.width = 200;
  canvas.height = 120;
  ...
  function drawCurve() {
    ...
    // Aproximación de la curva de saturación: tanh(x * drive/10)
    ctx.beginPath();
    ...
    for (let pixelX = 0; pixelX <= plotW; pixelX++) {
      ...
      const output = Math.tanh(input * drive / 10);
